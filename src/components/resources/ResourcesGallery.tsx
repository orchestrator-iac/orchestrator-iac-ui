...
): boolean => cloudFilter === "all" || resource.cloudProvider === cloudFilter;

const matchesSearchQuery = (resource: ResourceItem, query: string): boolean => {
  ...
  return (
    ...
    (resource.resourceId || "").toLowerCase().includes(term)
  );
};

const compareByName = (a: ResourceItem, b: ResourceItem): number =>
  (a.resourceName || "").localeCompare(b.resourceName || "");

const compareByNewest = (a: ResourceItem, b: ResourceItem): number => {
  const at = a.publishedAt ? new Date(a.publishedAt).getTime() : 0;
  const bt = b.publishedAt ? new Date(b.publishedAt).getTime() : 0;
  return bt - at || compareByName(a, b);
};
...
const compareByPopularity =
  ...
  (a: ResourceItem, b: ResourceItem): number => {
    const ac = usageById[a.resourceId] || 0;
    const bc = usageById[b.resourceId] || 0;
    return bc - ac || compareByName(a, b);
  };

const getResourceComparator = (
  ...
  return compareByPopularity(usageById);
};

// Fetches resolved icons for a batch of resources and reduces the results
// into a lookup map, skipping any resource whose icon failed to resolve.
const collectResolvedIcons = async (
  targets: ResourceItem[],
): Promise<Record<string, ResourceIconValue>> => {
  const entries = await Promise.all(
    targets.map(
      async (resource) =>
        [resource._id, await resolveResourceIcon(resource)] as const,
    ),
  );

  return entries.reduce<Record<string, ResourceIconValue>>(
...
const ResourcesEmptyState: React.FC<ResourcesEmptyStateProps> = ({
  searchTerm,
  canCreateResources,
  onClearFilters,
}) => {
    ...
    <Grid size={12}>
      <Fade in timeout={800}>
        ...
        >
          ...
          >
            <FontAwesomeIcon icon={searchTerm ? "search" : "cube"} />
          </Box>
          <Typography variant="h6" sx={{ fontWeight: 700, mb: 0.75 }}>
            {searchTerm
          ...
          >
            {searchTerm && "Try adjusting your search or cloud filter."}
            {!searchTerm &&
              canCreateResources &&
...
type ResourcesFilterBarProps = {
  ...
  cloudFilter: CloudFilter;
  onCloudFilterChange: (value: CloudFilter) => void;
  sortBy: ResourceSortBy;
  onSortByChange: (value: ResourceSortBy) => void;
  showResultCount: boolean;
  resultCount: number;
};

const ResourcesFilterBar: React.FC<ResourcesFilterBarProps> = ({
  showContent,
  localSearch,
  onSearchChange,
  cloudFilter,
  ...
  return (
    <Fade in={showContent} timeout={700}>
      ...
      >
        ...
        <Select
          value={cloudFilter}
          onChange={(e) => onCloudFilterChange(e.target.value as CloudFilter)}
          size="small"
        ...
        <Select
          value={sortBy}
          onChange={(e) => onSortByChange(e.target.value as ResourceSortBy)}
          size="small"
        ...
        {showResultCount && (
          <Chip
            label={`${resultCount} resource${resultCount === 1 ? "" : "s"}`}
            size="small"
            sx={{
              fontWeight: 600,
...
const ResourcesGallery: React.FC = () => {
  ...
  useEffect(() => {
    ...
    void collectResolvedIcons(targets).then((nextIcons) => {
  ...
  if (isLoading) {
    galleryContent = <LoadingSkeletonGrid />;
  } else if (isError) {
    galleryContent = (
      <ResourcesErrorState onRetry={() => dispatch(fetchResources())} />
    );
  } else if (isEmpty) {
    galleryContent = (
      <ResourcesEmptyState
  ...
  return (
    ...
    >
      ...
      <ResourcesFilterBar
        ...
        showResultCount={status === "succeeded"}
